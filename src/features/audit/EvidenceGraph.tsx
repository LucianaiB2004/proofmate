...
export function EvidenceGraph({ claim, evidence }: { claim: Claim; evidence: EvidenceItem[] }) {
  ...
  return (
    <section className="graph-panel" aria-labelledby="graph-title">
      <div className="panel-heading"><div><p className="section-kicker">EVIDENCE MAP</p><h2 id="graph-title">证据图谱</h2></div><span>支持关系</span></div>
      <div className="graph-scroll">
        <svg className="evidence-graph" viewBox="0 0 700 330" role="img" aria-label={`${claim.statement} 与 ${linked.length} 条证据的关系图`}>
          <defs><linearGradient id="nodeGlow"><stop stopColor="#ff1744"/><stop offset="1" stopColor="#9b0827"/></linearGradient></defs>
          {linked.map((item, index) => {
            ...
            return <g key={item.id}>
              ...
              <rect x={x - 62} y={y - 30} width="124" height="60" rx="10" className="evidence-node" />
